...
/**
 * Constructs a {@link HibernateBundle}, deferring provision of injected resources such as {@link
 * EntityContribution} until after injection.
 *
...
public class HibernateBundleFactory<T extends Configuration> {
  ...
  /**
   * Constructor. Initialise with a lambda which will provide an appropriately configured {@link
   * DataSourceFactory} from the application configuration.
   *
  ...
  /**
   * Constructs the deferred-initialisation {@link HibernateBundle} on first request, then returns
  ...
  private HibernateBundle<T> create() {
    ...
    // configuration
    bundle =
        new HibernateBundle<T>(ImmutableList.of(), sessionFactoryFactory) {
          @Override
